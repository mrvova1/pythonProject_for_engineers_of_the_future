...
class Predlojenia(QMainWindow):
    def __init__(self):
        ...
        self.zagr_pred()
        self.res = []
        # self.pushButton.clicked.connect(prinat())
        # self.pushButton_2.clicked.connect(otkaz())
    ...
    def prinat(self):
        ...
        b = None
        try:
            b = open(f'Blocks/{block}')
        except BaseException:
            pass
        if b == None:
            f = open(f'Tranzactions/{tranzakt}')
        else:
            pass
        tranzaction_creater({'hash_of_tranzaktion': f['hash'], 'sign_of_tranzaktion': f['sign'], 'sost': 'accepted'})
        pass
    # доделать itog
    # В процессе создания
...
def block_creater(*b):
    ...
    t1 = json.dumps(b, separators=(',', ':'), indent=4)
    f.write(t1)
    #В процессе создания
...
class Arenda(QMainWindow):
    ...
    def save_results(self):
        ...
        que = """INSERT INTO Arenda(number_vagona,type_vagona,date_start_arenda,date_end_arenda,stoimost,arendator) 
        VALUES(?,?,?,?,?,?)"""
        self.cur.execute(que, (number_vagon, typeCode, startArenda, endArenda, stoimost, organization_now))
        que2 = f"UPDATE Vagoni SET Arenduuschiy={organization_now} WHERE Number={number_vagon}"
        self.cur.execute(que2)
        self.con.commit()
        tranzaction_creater({'number_vagona': number_vagon, 'type_vagona': typeCode, 'date_start_arenda': startArenda,
...
class Nakladnaya(QMainWindow):
    ...
    def save_results(self):
        ...
        ,VesGruza,StartStation,OverStation,DateOtpravleniya,SrokDostavki,Stoimost) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)"""
        self.cur.execute(que, (numberNakladnoy, nVag, organization_now, gruzopo, gruzope, gruz, vesGruza, sOtp, sNaz,
                               dateStart, srokDostavki, stoimostPerevozki))
        self.con.commit()
        tranzaction_creater(
    ...
    def set_Gruzopoluchatel(self):
        ...
        for element in result:
            if element[2] == 3:
                ...
                self.gruzopo.addItem(element[1])
